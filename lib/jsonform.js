...
jsonform.elementTypes = {
  ...
  'radiobuttonset': {
      'template': ''
      + '<div id="<%= node.id %>"'
      + ' class="btn-group <%= elt.htmlClass ? " " + elt.htmlClass : "" %>">'
      + '   <% _.each(node.options, function(key, val) {%>'
      + '   <label class="<%= cls.buttonClass %>">'
      + '     <input type="radio" style="position:absolute;left:-9999px;"'
      + '       <%= (value === key.value || value === key) ? "checked=checked" : "" %>' 
      + '       name="<%= node.name %>"'
    ...
    onBeforeRender: function(evt, node) {
      if( node.schemaElement.type === 'boolean' ) {
        evt.value = (_.isBoolean( evt.value )) ? evt.value : null;
        
        if (!node.required || evt.value === null) {
          node.options.push({
            title: 'None',
            value: null,
          });
  ...
  'select2': {
    ...
    'onInsert': function(evt, node) {
      var defaultOptions = {};
      var options = _.extend( defaultOptions, node.formElement.select2options );

      $('#' + escapeSelector(node.id) ).select2( options );
    },
  },
